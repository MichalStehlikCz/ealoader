...
@Command(description = "Load catalogue data to Enterprise Architect database", name="ealoader",
        mixinStandardHelpOptions = true, version = "1.0")
public class EALoaderCliApplication implements Runnable {

    public static void main(String[] args) {
        int exitCode = new CommandLine(new EALoaderCliApplication())
                .registerConverter(Level.class, Level::valueOf)
                .execute(args);
        System.exit(exitCode);
    }

    @Option(names = {"-p", "--provysdb"},
    ...
    @Option(names = {"--provysuser"}, description = "Provys DB user", defaultValue = "ealoader")
    private String provysUser;

    @Option(names = {"--provyspwd"}, description = "Provys DB user password", defaultValue = "heslo")
    private String provysPwd;

    @Option(names = {"-e", "--eaproject"}, description = "Enterprise architect project",
    ...
    private void configureLogger() {
        ...
        final Logger logger = LogManager.getLogger(com.provys.ealoader.EALoaderCliApplication.class);
    ...
    public void run() {
        ...
        RunEALoader runner = container.select(RunEALoader.class).get();
